...
#define LCD_GPIO_EN			GPIOE

#define LCD_PIN_RS			GPIO_Pin_1   // Conectado a PE1
#define LCD_PIN_EN			GPIO_Pin_3   // Conectado a PE3

#if (USE_RW == 1)
#define LCD_GPIO_RW			GPIOE        // Conectado a PE2
#define LCD_PIN_RW			GPIO_Pin_2
#endif

//--------- DATA PINs (D4~D7) ------------
#define LCD_GPIO_D4			GPIOE        // Conectado a PE4
#define LCD_GPIO_D5			GPIOE        // Conectado a PE5
#define LCD_GPIO_D6			GPIOE        // Conectado a PE6
#define LCD_GPIO_D7			GPIOC        // Conectado a PC13 (Puerto C)
...
void LCD_init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOE, ENABLE);	// Enciende reloj del Puerto E (RS, RW, EN, D4, D5, D6)
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC, ENABLE);	// Enciende reloj del Puerto C (Para D7 en PC13)
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);	// Enciende reloj del Puerto D (Para el Backlight en PD8)
	...
	// --- Configuración del Pin de Backlight (PD8) ---
	GPIO_InitTypeDef GPIO_InitStructureBacklight;

	// Ya encendimos el reloj RCC_AHB1Periph_GPIOD al principio de LCD_init
	GPIO_InitStructureBacklight.GPIO_Pin = GPIO_Pin_8;
	GPIO_InitStructureBacklight.GPIO_Mode = GPIO_Mode_OUT;        // Salida digital
	GPIO_InitStructureBacklight.GPIO_OType = GPIO_OType_PP;       // Push-Pull
	GPIO_InitStructureBacklight.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructureBacklight.GPIO_PuPd = GPIO_PuPd_NOPULL;
	GPIO_Init(GPIOD, &GPIO_InitStructureBacklight);

	// Por defecto, lo encendemos al arrancar mandando un '1' lógico a la base del transistor
...
{
    GPIO_SetBits(GPIOD, GPIO_Pin_8); // Pone PD8 en HIGH -> Satura transistor -> Cátodo a Tierra (ON)
}

void LCD_Backlight_Off(void)
{
    GPIO_ResetBits(GPIOD, GPIO_Pin_8); // Pone PD8 en LOW -> Corta transistor -> Cátodo al aire (OFF)
}
